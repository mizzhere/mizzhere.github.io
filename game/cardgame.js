...
function saveState() {
    try {
        const stateToSave = {
            ...
            // Dữ liệu Play Pass
            specialCurrency: player.specialCurrency,
            playPassXP: player.playPassXP,
            hasPaidPass: player.hasPaidPass,
            claimedPassRewards: player.claimedPassRewards
        };
...
function loadState() {
    try {
        ...
        if (savedState) {
            ...
            // Tải dữ liệu Play Pass
            player.specialCurrency = loadedState.specialCurrency || 0;
            player.playPassXP = loadedState.playPassXP || 0;
            player.hasPaidPass = loadedState.hasPaidPass || false;
            player.claimedPassRewards = loadedState.claimedPassRewards || {};
...
let gameData = {}; // Sẽ chứa dữ liệu từ gamedata.json
let cardDatabase = [];
...
const currencyIcons = {
    ...
    specialCurrency: 'https://placehold.co/24x24/67e8f9/0e7490?text=G',
    chest: 'https://placehold.co/48x48/ca8a04/713f12?text=R'
};
...
const createCardHTML = (card, options = {}) => {
    ...
    return `
    <div class="game-card ${rarityClass} ${unownedClass} ${lockedClass}" data-card-id="${card.id}">
        ${unownedOverlay}
...
// --- PLAY PASS RENDER FUNCTIONS ---
const createRewardItemHTML = (level, reward, type) => {
    if (!reward) return '<div class="pass-reward-item empty"></div>';
    const rewardKey = `${level}-${type}`;
    const isClaimed = player.claimedPassRewards[rewardKey];
    const canClaim = player.playPassLevel >= level && (type === 'free' || (type === 'paid' && player.hasPaidPass));
    let iconHtml = '';
    let amountHtml = '';
    if (reward.type === 'chest') {
        iconHtml = `<img src="${reward.icon}" class="w-12 h-12">`;
        amountHtml = `<span class="text-xs font-semibold text-gray-300">${reward.name}</span>`;
    } else {
        iconHtml = `<img src="${currencyIcons[reward.type]}" class="w-10 h-10 rounded-full">`;
        amountHtml = `<span class="font-bold text-white">${reward.amount.toLocaleString('vi')}</span>`;
    }
    const isLocked = type === 'paid' && !player.hasPaidPass;
    const lockOverlay = isLocked ? '<div class="locked-overlay"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 1a4.5 4.5 0 00-4.5 4.5V9H5a2 2 0 00-2 2v6a2 2 0 002 2h10a2 2 0 002-2v-6a2 2 0 00-2-2h-.5V5.5A4.5 4.5 0 0010 1zm3 8V5.5a3 3 0 10-6 0V9h6z" clip-rule="evenodd" /></svg></div>' : '';
    let buttonHtml = '';
    if (isClaimed) {
        buttonHtml = '<button class="claim-btn claimed" disabled>Đã nhận</button>';
    } else if (canClaim) {
        buttonHtml = `<button class="claim-btn can-claim" data-level="${level}" data-type="${type}">Nhận</button>`;
    } else {
        buttonHtml = '<button class="claim-btn" disabled>Nhận</button>';
    }
    return `
        <div class="pass-reward-item ${type}-track ${isLocked ? 'locked' : ''} ${isClaimed ? 'is-claimed' : ''}">
...
const renderPlayPassScreen = () => {
    player.playPassLevel = Math.floor(player.playPassXP / gameData.playPass.xpPerLevel) + 1;
    const currentLevelXP = (player.playPassLevel - 1) * gameData.playPass.xpPerLevel;
    const xpProgress = player.playPassXP - currentLevelXP;
    const progressPercentage = (xpProgress / gameData.playPass.xpPerLevel) * 100;
    const purchaseButtonHTML = player.hasPaidPass
        ? `<div class="font-bold text-lg text-yellow-400">Đã mở khóa Premium Pass</div>`
        : `<button id="purchase-pass-btn" class="bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-6 rounded-full flex items-center gap-2">
            <span>Mở khóa Premium (${gameData.playPass.price.toLocaleString('vi')})</span>
            <img src="${currencyIcons.tokens}" class="w-6 h-6 rounded-full">
           </button>`;
    let rewardsHTML = gameData.playPass.rewards.map(data => `
        <div class="pass-level-container ${player.playPassLevel >= data.level ? 'unlocked' : ''}">
            <div class="level-marker">
                <span class="level-number">${data.level}</span>
            </div>
            <div class="rewards-wrapper">
                ${createRewardItemHTML(data.level, data.freeReward, 'free')}
                ${createRewardItemHTML(data.level, data.paidReward, 'paid')}
            </div>
        </div>
    `).join('');
    return `
    <div class="animate__animated animate__fadeInUp flex flex-col h-full">
        <div class="pass-header">
            <h1 class="text-3xl font-black text-white">PLAY PASS</h1>
            <div class="pass-progress-container">
                <div class="level-info">
                    <span>Cấp ${player.playPassLevel}</span>
                    <span>${player.playPassXP.toLocaleString('vi')} / ${(player.playPassLevel * gameData.playPass.xpPerLevel).toLocaleString('vi')} XP</span>
                </div>
                <div class="progress-bar-bg">
                    <div class="progress-bar-fg" style="width: ${progressPercentage}%"></div>
                </div>
            </div>
            <div class="pass-purchase">
                ${purchaseButtonHTML}
                 <button id="add-xp-btn" class="bg-blue-500 text-white p-2 rounded-full text-xs">Thêm 25 XP (Test)</button>
            </div>
        </div>
        <div id="pass-rewards-grid" class="flex-grow overflow-y-auto pr-2">
            ${rewardsHTML}
        </div>
    </div>`;
};
...
async function loadGameData() {
    try {
        const [cardsResponse, shopResponse, gameDataResponse] = await Promise.all([
            fetch('cards.json'),
            fetch('shop.json'),
            fetch('gamedata.json')
        ]);
        ...
        if (!gameDataResponse.ok) throw new Error(`Không thể tải gamedata.json: ${gameDataResponse.status}`);
        ...
        console.log(`Tải thành công: ${cardDatabase.length} thẻ, ${shopItems.length} vật phẩm chính, ${eventShopItems.length} vật phẩm sự kiện, và dữ liệu game.`);
    } catch (error) {
...
function renderContent(page, subpage) {
    ...
    switch (page) {
        ...
        case 'pass': content = renderPlayPassScreen(); break;
...
function addEventListeners(page) {
    if (page === 'main') {
        ...
        document.getElementById('purchase-pass-btn')?.addEventListener('click', purchasePass);
        document.getElementById('add-xp-btn')?.addEventListener('click', () => addXP(25));
        document.querySelectorAll('.claim-btn.can-claim').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const level = parseInt(e.currentTarget.dataset.level);
                const type = e.currentTarget.dataset.type;
                claimPlayPassReward(level, type);
            });
        });
    } else if (page === 'game') {
...
// --- PLAY PASS LOGIC FUNCTIONS ---
function claimPlayPassReward(level, type) {
    const rewardKey = `${level}-${type}`;
    if (player.claimedPassRewards[rewardKey]) return;
    const data = gameData.playPass.rewards.find(d => d.level === level);
    if (!data) return;
    const reward = type === 'free' ? data.freeReward : data.paidReward;
    if (!reward) return;
    if (reward.type === 'chest') {
        showPurchaseResultModal(`Mở ${reward.name}`, [{ type: 'custom', item: { name: reward.name, icon: reward.icon } }]);
    } else {
        player[reward.type] += reward.amount;
        const received = [{ type: 'currency', item: { currencyType: reward.type, amount: reward.amount } }];
        showPurchaseResultModal('Nhận thưởng Play Pass', received);
    }
    player.claimedPassRewards[rewardKey] = true;
    updateCurrencyDisplay();
    saveState();
    renderContent('pass');
}
function purchasePass() {
    if (player.tokens < gameData.playPass.price) {
        alert('Không đủ Token để mua Premium Pass!');
        return;
    }
    if (confirm(`Bạn có chắc muốn dùng ${gameData.playPass.price.toLocaleString('vi')} Token để mở khóa Premium Pass?`)) {
        player.tokens -= gameData.playPass.price;
        player.hasPaidPass = true;
        updateCurrencyDisplay();
        saveState();
        renderContent('pass');
    }
...
function showPurchaseResultModal(itemName, receivedItems) {
    let itemsHTML = receivedItems.map(received => {
        if (received.type === 'card') {
            ...
            `;
        } else if (received.type === 'custom') { // Xử lý cho rương
            return `
                <div class="flex flex-col items-center justify-center bg-gray-800 rounded-lg p-4 gap-2">
                    <img src="${received.item.icon}" class="w-16 h-16">
                    <span class="font-bold text-md text-center text-white">${received.item.name}</span>
                </div>
            `;
        }
        return '';
    ...
    document.getElementById('purchase-result-content').innerHTML = `
        <div class="p-4 border-b border-white/10"><h2 class="text-xl font-bold text-white text-center">${itemName}</h2></div>
...
function updateCurrencyDisplay() {
    ...
    document.getElementById('player-special-currency').innerHTML = `<img src="${currencyIcons.specialCurrency}" class="currency-icon"><span>${player.specialCurrency.toLocaleString('vi')}</span>`;
}
...
function redrawGameBoard() {
    ...
    slotContainers.forEach((slot, i) => {
        ...
        if (cardId) {
            const cardInfo = findCard(cardId);
            slot.insertAdjacentHTML('afterbegin', createCardHTML(cardInfo, { simple: true, showAttack: true }));
        }
        if (gameState.selectedCardId && !cardId) {
